...
class MLModel:
    ...
    def inference(self, state_dicts, scaler):
        ...
        # Compute errors
        rmse = np.sqrt(mean_squared_error(mean_values, self.df_10[self.target_col]))
    ...
    def apply_model(
        ...
        # 3D mesh
        depth_vals = ds[z].values
        y_vals     = ds[y].values
        x_vals     = ds[x].values

        depth3d, y3d, x3d = np.meshgrid(depth_vals, y_vals, x_vals, indexing="ij")

        # Append flattened metadata
        var_flat.append(x3d.ravel(order="C"))
        var_flat.append(y3d.ravel(order="C"))
        var_flat.append(depth3d.ravel(order="C"))

        
        var_flat = [
            ds["so"].values.ravel(order="C"),
            ds["thetao"].values.ravel(order="C"),
            x3d.ravel(order="C"),
            y3d.ravel(order="C"),
            depth3d.ravel(order="C")
        ]
